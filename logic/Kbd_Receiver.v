`timescale 1ns / 1ps
//////////////////////////////////////////////////////////////////////////////////
// 适配说明：CH374T芯片、3.3V供电、无HUB、单键盘设备
//////////////////////////////////////////////////////////////////////////////////
module Kbd_Receiver
#(
    // 系统参数
    ...
    parameter   REPORT_LEN              = 8'd8,
    parameter   USEFUL_DATA_LEN         = 8'd8
)
(
    // 系统时钟与复位
    input                   clk,                    // 系统时钟50MHz
    ...
    output reg [7:0]  o_kbd_modifiers,
    output reg [47:0] o_kbd_keys,
    output reg        o_kbd_data_valid
);
...
localparam  S_ENUM_SET_PROTO_SETUP   = 8'h13; // 发送 Set_Protocol 设定包
localparam  S_ENUM_SET_PROTO_WAIT    = 8'h14; // 等待 Setup 传输完成
localparam  S_ENUM_SET_PROTO_IN      = 8'h15; // 发送 IN 令牌(状态阶段)
localparam  S_ENUM_SET_PROTO_WAIT_IN = 8'h16; // 等待 IN 传输完成

// ----- 轮询状态群 -----
localparam  S_POLL_WAIT             = 8'h17; // 等待X ms定时器
localparam  S_POLL_EP1_ISSUE        = 8'h18; // 发送IN令牌+启动传输
localparam  S_POLL_EP1_WAIT_INT     = 8'h19; // 等待INT#低电平（非脉冲模式）
localparam  S_POLL_EP1_READ         = 8'h1A; // 读取键盘数据
...
reg  [7:0]  spi_rdata_buf[USEFUL_DATA_LEN - 1:0];
...
genvar i;
generate
    for(i = 0; i < USEFUL_DATA_LEN; i = i + 1) begin : gen_kbd_data
...
always @(*) begin
    case(curr_state)
        ...
        S_ENUM_SET_CONF_WAIT_IN: next_state = (op_step == OP_FINISH) ? S_ENUM_SET_PROTO_SETUP           : ((op_step == OP_RESET) ? S_ENUM_SET_CONF_IN : (timeout_flag ? S_ERROR : S_ENUM_SET_CONF_WAIT_IN));        
        
        // Set_Protocol 转移逻辑
        S_ENUM_SET_PROTO_SETUP:   next_state = (op_step == OP_FINISH) ? S_ENUM_SET_PROTO_WAIT    : (timeout_flag ? S_ERROR : S_ENUM_SET_PROTO_SETUP);
        S_ENUM_SET_PROTO_WAIT:    next_state = (op_step == OP_FINISH) ? S_ENUM_SET_PROTO_IN      : (timeout_flag ? S_ERROR : S_ENUM_SET_PROTO_WAIT);
        S_ENUM_SET_PROTO_IN:      next_state = (op_step == OP_FINISH) ? S_ENUM_SET_PROTO_WAIT_IN : (timeout_flag ? S_ERROR : S_ENUM_SET_PROTO_IN);
        S_ENUM_SET_PROTO_WAIT_IN: next_state = (op_step == OP_FINISH) ? S_POLL_WAIT              : ((op_step == OP_RESET) ? S_ENUM_SET_PROTO_IN : (timeout_flag ? S_ERROR : S_ENUM_SET_PROTO_WAIT_IN));
...
always @(posedge clk) begin
    if(!rst_n) begin
        ...
        o_kbd_modifiers <= 8'd0;
        o_kbd_keys      <= 48'd0;
        o_kbd_data_valid <= 1'b0;
        delay_ms_cnt <= 8'd0;
    end else begin
        ...
        case(curr_state)
            ...
            // === 第一组：等待 SETUP 传输完成 ===
            S_ENUM_SET_ADDR_WAIT, S_ENUM_SET_CONF_WAIT, S_ENUM_SET_PROTO_WAIT: begin
                if(spi_req_ready && !spi_req_valid && op_step == 4'd0) begin
                    spi_read_reg(REG_INTER_FLAG, 8'd1);
                    op_step <= 4'd1;
                end else if(spi_resp_valid && op_step == 4'd1) begin
            ...
            // === 第二组：等待 IN 状态阶段完成（经常被 NAK，必须读状态并处理重试） ===
            S_ENUM_SET_ADDR_WAIT_IN, S_ENUM_SET_CONF_WAIT_IN, S_ENUM_SET_PROTO_WAIT_IN: begin
                if(spi_req_ready && !spi_req_valid && op_step == 4'd0) begin
                    spi_read_reg(REG_INTER_FLAG, 8'd1);
                    op_step <= 4'd1;
                end else if(spi_resp_valid && op_step == 4'd1) begin
            ...
            S_ENUM_SET_ADDR_IN, S_ENUM_SET_CONF_IN, S_ENUM_SET_PROTO_IN: begin
                // 控制传输状态阶段：下发 IN 令牌
                if(spi_req_ready && !spi_req_valid && op_step == 4'd0) begin
            ...
            // ---------------- USB枚举：Set Protocol (强制引导模式) ----------------
            S_ENUM_SET_PROTO_SETUP: begin
                if(spi_req_ready && !spi_req_valid && op_step == 4'd0) begin
                    // 步骤1：写入 Set_Protocol(Boot) 控制包
                    // {wLength_H, wLength_L, wIndex_H, wIndex_L, wValue_H, wValue_L, bRequest(0x0B), bmRequestType(0x21)}
                    spi_write_multi(RAM_HOST_TRAN, {8'h00, 8'h00, 8'h00, 8'h00, 8'h00, 8'h00, 8'h0B, 8'h21});
                    op_step <= 4'd1;
                end else if(spi_resp_valid && op_step == 4'd1) begin
                    op_step <= 4'd2;
                end else if(spi_req_ready && !spi_req_valid && op_step == 4'd2) begin
                    ...
                    spi_write_reg(REG_USB_H_TOKEN, 8'hD0); // SETUP 令牌发给 EP0
                    op_step <= 4'd5;
                end else if(spi_resp_valid && op_step == 4'd5) begin
                    op_step <= 4'd6;
                end else if(spi_req_ready && !spi_req_valid && op_step == 4'd6) begin
                    spi_write_reg(REG_USB_H_CTRL, 8'h08);  // 启动传输 (DATA0)
                    op_step <= 4'd7;
                end else if(spi_resp_valid && op_step == 4'd7) begin
                    op_step <= OP_FINISH;
                end
            ...
            S_POLL_EP1_ISSUE: begin
                if(spi_req_ready && !spi_req_valid && op_step == 4'd0) begin
                    // 步骤1：下发 IN 令牌到 EP2 (PID=0x9, EP=0x1 -> 0x91)
            ...
            S_POLL_EP1_READ: begin
                if(spi_req_ready && !spi_req_valid && op_step == 4'd0) begin
                    ...
                    // 解析键盘数据
                    o_kbd_modifiers <= spi_resp_rdata[0];
                    // spi_resp_rdata[1] 是保留位，跳过
                    o_kbd_keys      <= {spi_resp_rdata[7], spi_resp_rdata[6], 
                                        spi_resp_rdata[5], spi_resp_rdata[4], 
                                        spi_resp_rdata[3], spi_resp_rdata[2]};
                    
                    o_kbd_data_valid <= 1'b1;
                    ep1_in_toggle <= ~ep1_in_toggle;
                    op_step <= OP_FINISH;
                end
            end
